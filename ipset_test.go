	...
	for _, group := range ipGroups {
		t.Run(group.name, func(t *testing.T) {
			...
			for _, cidr := range group.cidrs {
				low, high, err := getHostsRangeFromCIDRString(cidr.String())
				if err != nil {
...
func getHostsRangeFromCIDRString(s string) (uint32, uint32, error) {
...
func getNestedSubnets(subnet *net.IPNet) (res []*net.IPNet) {
	if subnet == nil {
		return
	}

	maskSize, maskBits := subnet.Mask.Size()
	for i := maskSize; i <= maskBits; i++ {
		res = append(res, &net.IPNet{IP: subnet.IP, Mask: net.CIDRMask(i, maskBits)})
	}
...
var subnetGroups = []struct {
	name            string
	cidrs           []*net.IPNet
	negativeSubnets []*net.IPNet
}{
	// {
	// 	name:            "one block enclosing another",
	// 	cidrs:           parseCidrs("192.168.0.0/25", "192.168.0.0/24"),
	// 	negativeSubnets: parseCidrs("184.0.0.1/24"),
	// },
	// {
	// 	name:            "two overlapping blocks",
	// 	cidrs:           parseCidrs("255.0.0.0/20", "254.0.0.0/20"),
	// 	negativeSubnets: parseCidrs("253.0.0.1/19"),
	// },
	// {
	// 	name:            "three overlapping blocks",
	// 	cidrs:           parseCidrs("255.0.0.0/20", "254.0.0.0/20", "128.0.0.0/20"),
	// 	negativeSubnets: parseCidrs("253.0.0.1/20", "84.0.0.1/20"),
	// },
	// {
	// 	name:            "2 unrelated /32 blocks",
	// 	cidrs:           parseCidrs("127.0.0.1/32", "127.0.0.2/32"),
	// 	negativeSubnets: parseCidrs("127.0.0.3/32", "32.0.0.1/32"),
	// },
	// {
	// 	name:            "all previous mixed",
	// 	cidrs:           parseCidrs("192.168.0.0/25", "192.168.0.0/24", "255.0.0.0/20", "254.0.0.0/20", "128.0.0.0/20", "127.0.0.1/32", "127.0.0.2/32"),
	// 	negativeSubnets: parseCidrs("184.0.0.1/24", "253.0.0.1/19", "84.0.0.1/20", "127.0.0.3/32", "32.0.0.1/32"),
	// },
	// {
	// 	name:            "0 vs 1 prefix",
	// 	cidrs:           parseCidrs("255.0.0.0/20", "128.0.0.0/20"),
	// 	negativeSubnets: parseCidrs("254.0.0.2/20", "254.0.0.129/20", "129.0.0.2/20", "129.0.0.129/20"),
	// },
	// {
	// 	name: "de-aggregated multi-prefix policy",
	// 	cidrs: func() []*net.IPNet {
...
func TestContainsSubnet(t *testing.T) {
	...
	for _, group := range subnetGroups {
		t.Run(group.name, func(t *testing.T) {
			...
			for _, subnet := range group.negativeSubnets {
				for _, sub := range getNestedSubnets(subnet) {
					t.Run(sub.String(), func(t *testing.T) {
						if got := s.ContainsSubnet(sub); got != false {
							t.Errorf("negative case returned true: %s", subnet.String())
						}
					})
				}
			}
			...
			for _, cidr := range group.cidrs {
				for _, nestedSubnet := range getNestedSubnets(cidr) {
					t.Run(nestedSubnet.String(), func(t *testing.T) {
						if s.ContainsSubnet(nestedSubnet) != true {
							t.Errorf("positive case returned false: %s", nestedSubnet.String())
						}
					})
				}
			}
